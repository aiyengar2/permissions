...
func (m AccessMasks) ToExplicitAccess() []windows.EXPLICIT_ACCESS {
	return m.ToExplicitAccessCustom(nil, nil, nil)
}

func (m AccessMasks) ToExplicitAccessCustom(owner, group, everyone *windows.SID) []windows.EXPLICIT_ACCESS {
	if owner == nil {
		owner = sid.CreatorOwner()
	}
	if group == nil {
		group = sid.CreatorGroup()
	}
	if everyone == nil {
	...
	return []windows.EXPLICIT_ACCESS{
		...
		access.GrantSid(m.Everyone, everyone),
	}
}
